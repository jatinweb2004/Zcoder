...
const OpenQueries = () => {
    ...
    const fetchDetailQuery = async (postId) => {
        try {
            const response = await axios.get(`http://localhost:8051/api/getdetailquerybyid/${postId}`);
    ...
    const fetchProfile = async (userId) => {
        try {
            const response = await axios.get(`http://localhost:8051/api/profile/${userId}`);
    ...
    const fetchComments = async (queryId) => {
        try {
            const response = await axios.get(`http://localhost:8051/api/querycomments/${queryId}`);
    ...
    const handleCommentSubmit = async () => {
        if (newComment.trim()) {
            try {
                const response = await axios.post("http://localhost:8051/api/querycomments", {
                    queryId: id,
    ...
    useEffect(() => {
        async function fetchMyQueries() {
          try {
            ...
            const response = await axios.get(`http://localhost:8051/api/getreatedqueries/${detailQuery.category}`);
