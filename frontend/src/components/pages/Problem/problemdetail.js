...
const Problem = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8051/api/profile/${user.uid}`)
      .then((Profile) => {
    ...
    const fetchProblemDetail = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8051/api/Problem/${problemId}`
        );
  ...
  useEffect(() => {
    if (problems && problems.length > 0 && problems[0].email) {
      const fetchProfileDetails = async () => {
        try {
          const response = await axios.get(
            `http://localhost:8051/api/ownerprofile/${problems[0].email}`
          );
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8051/api/comments/${problemId}`
        );
  ...
  const handleCommentSubmit = async () => {
    try {
      const response = await axios.post("http://localhost:8051/api/comments", {
        problemId,
  ...
  useEffect(() => {
    const fetchLikeStatus = async () => {
      try {
        const response = await axios.get(`http://localhost:8051/api/problemslike/status/${problemId}/${user.uid}`);
  ...
  const handleLikeClick = async () => {
    try {
      const response = await axios.post(`http://localhost:8051/api/problemslike/${problemId}/${user.uid}/like`);
      if (response.status === 200) {
  ...
  const handleCollaboration = async () => {
    try {
      ...
      await axios.post("http://localhost:8051/api/send-collab-request", requestBody);
